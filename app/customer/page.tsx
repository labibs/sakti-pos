...
import { Search, ShoppingCart, Loader2, Store } from "lucide-react";
...
export default function CustomerHomePage() {
  ...
  const { cart, addToCart, updateQty, merchantId, setMerchantId, tableNumber } =
    useCustomerStore();

  // Ambil merchant ID dari URL atau store
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // Log error jika ada salah satu yang gagal tanpa menghentikan yang lain
        [resItems, resCats, resMerchant].forEach((res, i) => {
    ...
    return (
      <div className="min-h-screen bg-[#F9FAFB] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-sage-400 font-bold mb-8 uppercase tracking-widest text-[10px]">
          Silakan scan QR code di meja atau gunakan link yang valid
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-screen bg-[#F9FAFB] pb-48">
      ...
      {merchantInfo && (
        <div className="bg-white px-4 py-3 flex items-center justify-between border-b border-line/30">
          ...
          {isSignedIn && tableNumber && (
            <div className="flex items-center gap-1.5 bg-sage-50 px-3 py-1.5 rounded-xl border border-line">
              <span className="text-[9px] font-black text-sage-700 uppercase tracking-widest">
                Meja {tableNumber}
              </span>
            </div>
          )}
      ...
      <div className="px-4 py-6">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20">
            <p className="text-sage-400 font-bold uppercase tracking-widest text-xs">
      ...
      {itemCount > 0 && (
        <div className="fixed bottom-24 left-4 right-4 z-40 sm:left-1/2 sm:right-auto sm:w-[480px] sm:-translate-x-1/2">
          <Link href={`/customer/cart?m=${currentMerchantId}`}>
